...
def provision_run_workspace(*, base_repo: Path, runs_dir: Path, run_id: str, git: str = "git") -> Path:
    ...
    if workspace.exists():
        if is_git_repo(workspace, git=git):
            return workspace  # fully provisioned before — keep the run's work
        raise GitError(f"existing run workspace is not a valid git repo (partial/corrupt): {workspace}")
    ...
    try:
        ...
        _git(["-C", str(staging), "checkout", "-b", run_branch_name(run_id)], git=git, check=True)
