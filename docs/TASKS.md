...
- [x] `LLMGateway` implements the existing `AgentRuntime` protocol (drop-in for MockAgentRuntime; engine loop unchanged); 15 tests against fake adapters — strategy selection/memoization, submit_result extraction, guided read-from-text, free-text-reasoning capture, budget-exhaustion forcing, tool loop, normalized-transcript shape, idempotency-key-before-execution, repair loop, per-entry retry + fallback chain, per-endpoint client caching, temperature-drop for no-sampling-param models, prompt assembly
...
- [ ] **Deferred to the toolkit slice:** wire the gateway into the CLI (`run start --backend llm`) with real secrets/endpoint resolution — needs real credentials + real tool execution to smoke-test end-to-end. `ToolExecutor` is the injection seam (no-op in this slice); the idempotency-key parameter is already on its signature so real connectors slot in without a protocol change.
...
- [ ] Surface an agent's real toolkits as callable tools to the model in the gateway (today the gateway offers only `submit_result`) — needed before the model can actually invoke any of the above end-to-end; pairs with the CLI wiring.
