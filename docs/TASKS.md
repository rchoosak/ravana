...
- [x] **Wire the gateway into the CLI** (`run start/watch/hitl respond --backend llm`): builds `EnvSecretResolver` → `build_registry` → `RavanaToolExecutor` → provider adapters (selected per the graph's `llm.provider`s: `anthropic` → AnthropicAdapter, everything else → OpenAICompatibleAdapter reached via `llm.endpoint`) → `LLMGateway`. `--mock-fixture` is now required only for `--backend mock`. **Endpoint** resolution is real (flows through to the adapter as `base_url`); **toolkit** `auth_ref`s resolve via `EnvSecretResolver` (§8c). *Still partial:* **LLM provider credentials** come from each SDK's own env var (`ANTHROPIC_API_KEY`, …) / the `api_key_ref` pass-through in the adapters — resolving `llm.api_key_ref` through `EnvSecretResolver` too is a tracked follow-up. Unit-tested at the wiring level (provider→adapter selection, gateway construction with toolkits, mock-requires-fixture); the live end-to-end run needs real credentials and is a manual smoke test (below).
