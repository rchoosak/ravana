...
- **Non-transient failures** (repair budget exhausted, tool auth failure, sandbox crash, guard exceeded): `node_execution.status = FAILED` → `run.status = FAILED`. Full `shared_state`, message trail and error are preserved (nothing is rolled back) so an engineer can inspect and manually resume from a specific node rather than restart the whole `Run`. *(Phase 0b: enforced by type — repair exhaustion and the bounded-turn ceiling raise `AgentOutputError` (not a transient), so the run fails immediately instead of spending `max_retries_per_node` re-asking a model past an expired repair budget. Tool failures are typed on `ToolkitError`: transport timeout / HTTP 5xx / 429 / 408 are **transient** (the turn ends as transient and the engine retries a fresh attempt with backoff — the §3.6 "tool timeout" row above; re-issued side effects dedupe on the content-addressed key); HTTP 401/403 is **fatal** (the "tool auth failure" here — fails the run; neither the model nor a retry fixes credentials); everything else (404/422, invalid arguments, bad path) is **model-addressable** — fed back into the turn as an error tool_result so the model can adjust or route around it.)*
