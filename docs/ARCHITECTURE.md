...
>
> **v0.10 changelog:** corrects a real flaw introduced in v0.9 — §10.1 said the workspace *is* the project directory, with `code_interpreter` mounting it read-write, which directly contradicts not impacting the developer's source. Agents now operate on an isolated `git clone --no-hardlinks` (or opt-in `git worktree`) per run, on its own branch, with the sandbox mount scoped to only that clone; handoff back to the real project is a PR/patch, never an auto-merge — local execution stays the right call, only what it's allowed to touch changes.
>
...
>
> **v0.15 changelog:** an external review pass fixed six findings. The original attempt-based idempotency key was first replaced by a command-content key; §3.6 now records the later correction to logical visit + call ordinal, which also distinguishes intentional identical calls and later graph re-entry. The pass also clarified sandbox wording, enforced graph-table constraints, made state-transition replay verifiable, normalized `auth_ref` placement, and split Phase 0 into deterministic-engine and real-integration slices.
>
...
- **Agent Definition vs. Node Execution Contract** — The Agent Definition is the reusable persona/model/skill configuration and the maximum Toolkit allow-list. Task-specific behavior lives on the graph node: the node grants a least-privilege Toolkit subset and owns its `output_schema` and HITL policy. Existing workflow files may inherit the Agent defaults, but the compiled graph always resolves one explicit Node Execution Contract before dispatch.
...
- **Execution Resource Scope** — The Agent Runtime owns provider adapters and Toolkit executors as async resources. SDK clients live in bounded concurrency-safe caches, close on eviction, and all surviving provider/HTTP clients close when the Runtime scope exits. CLI commands and future workers must close that scope in `finally`; resource cleanup is part of the interface, not an adapter convention.
...
- **No new workflow schema needed.** An MCP server is just another `toolkit.type` value — `mcp_server` — alongside the existing `web_search | code_interpreter | db | api_connector` (§2.2). `config` holds a server reference for `stdio` or a hosted `http`/SSE endpoint reference, plus an optional `allowed_tools` narrowing; the install/org registry owns the complete stdio command/args/env definition and a required absolute `cwd`, the server-specific `auth_env` name, whether discovery requires authentication, and which tools are explicitly read-only. The runtime discovers only MCP servers granted to the active node, validates provider-compatible qualified names, and persists an internal per-Run tool snapshot so a HITL resume in a fresh process reuses the same pinned list. `auth_ref` stays the toolkit's own top-level column exactly as for any other Toolkit type — not nested inside `config`, so the compiler/DB mapping doesn't have to special-case where credentials live per toolkit type. One `mcp_server`-type Toolkit exposes however many individual tools that server offers (or a narrowed subset) to the agent it's attached to — same attachment mechanism as any other Toolkit, no special case.
...
CREATE TABLE agent (
    ...
    max_tokens      INTEGER,
    output_schema   JSONB,                       -- legacy/default structured-output contract; a workflow_node may override it
    toolkit_ids     UUID[] NOT NULL DEFAULT '{}', -- capability ceiling; each workflow_node grants a subset
...
CREATE TABLE workflow_node (
    ...
    toolkit_ids     UUID[] NOT NULL DEFAULT '{}', -- least-privilege grants for this task; subset of agent.toolkit_ids
    hitl_config     JSONB,                        -- {enabled, trigger_condition, prompt_template, assignee} — assignee added §3.1
    output_schema   JSONB,                        -- task-specific state_delta contract; may inherit the agent default
...
CREATE TABLE node_execution (
    ...
    logical_visit_id UUID NOT NULL,                -- stable across retries/HITL resume; fresh when the graph enters this node again
...
CREATE TABLE message (
    ...
    tool_calls        JSONB,                         -- each entry includes logical_visit_id, call ordinal, and idempotency_key (§3.6)
...
- `workflow.status` + `run.workflow_version` and the validated `run.workflow_snapshot` captured at creation mean editing a workflow never affects `Run`s already in flight — resume compiles the immutable snapshot rather than locating mutable YAML, so there is no "live migration" of an executing graph. A `DRAFT` row is mutable and edited in place (so the Visual Studio doesn't spawn a new version per keystroke); the moment it's `PUBLISHED` it becomes immutable and any further edit must go through `POST /v1/workflows` again to create the next version (§7) — `PUBLISHED`/`ARCHIVED` rows never change underneath a `Run` that's already pinned to them.
...
4. **Commit** — Orchestrator re-reads the latest state/version, merges `state_delta`, and atomically marks `node_execution` successful, appends the `message`/`artifact` rows, performs the `state_version` CAS (§3.5), and writes the sequenced `state_transition_log` entry. A CAS conflict rolls the whole unit back, rebases the delta on the new state, and retries. This is the durability boundary: no observer can see successful turn data paired with stale state, and concurrent broadcast commits cannot silently clobber one another.
...
7. **Terminate** — When a node's outgoing edge targets `__terminal__`, **or a node has no outgoing edges at all** (an *implicit terminal* — equivalent to an explicit `to: [__terminal__]` edge, logged as a TERMINATE event; previously undefined, which produced a real stuck-`RUNNING` bug in Phase 0a), `run.status = COMPLETED`; for a sub-workflow run, `COMPLETED` also triggers the parent node's `output_map` (§1.8) to commit into the parent's `shared_state`, unblocking the parent's own step 5. If `guards.max_total_steps` is exceeded or an unhandled error occurs, `run.status = FAILED`. **`definition_of_done` is a termination gate (Phase 0b): reaching a terminal is necessary but not sufficient — the run `COMPLETED`s only if its DoD is met, else it `FAILED`s with the unmet criteria named, and a `DOD_EVALUATED` event records the outcome either way.** A criterion is classified by **parse**, not by evaluation: a criterion that parses as a boolean expression (e.g. `state.qa_status == 'PASS'`) is an *expression*, evaluated deterministically through the same sandboxed condition engine used for edges — and one that parses but *errors* at evaluation (e.g. an ordering comparison on an unset key, where `None > 5` raises) is a **failing** expression (unmet, gating), not prose. Only a criterion that fails to parse — a natural-language sentence like §4's "all acceptance criteria met" — is a *prose* criterion needing the `evaluated_by` agent's judgement. The evaluator itself is **pure and synchronous** (it only classifies and resolves expressions); prose judgement is I/O and lives at the engine boundary, which fetches the prose criteria, obtains a verdict, and applies it. A prose verdict is supplied by an injectable async function returning a **position-aligned** `ProseJudgement` — `verdicts[i]` rules on `prose_criteria[i]`, so criterion *identity* is carried by index, never by (collidable) criterion text. **Under the real LLM backend that verdict IS the LLM Gateway's `judge_prose`: it runs the `evaluated_by` agent once against the final `shared_state` — through the same §3.6 fallback chain a node uses, so a primary-provider outage yields to a fallback rather than failing the run — forcing a per-criterion `{index, met}` structured verdict (guided decoding where the provider supports it, else a forced `submit_verdict` tool that must arrive as *exactly one* call — several would let a second silently override the first), and fails *closed* at every seam: a criterion is met only when its verdict is *exactly* boolean `true`; an omitted, garbled, non-boolean, out-of-range, bool (a `false` index must not collide with criterion 0), or duplicated verdict reads as not-met, and a judgement that exhausts its repair budget or raises fails the run rather than completing it.** The gate wraps the *entire* obtain→meter→apply block, so a verdict that raises OR returns something malformed (a `None`, a bad shape) fails the run closed rather than escaping and stranding it at `RUNNING`; the failure carrier (`ProseJudgementError`) validates its `usage` is a real `LLMUsage` at construction, and the handler still guards against a mutated one, so recording a failed judgement's cost can't itself throw past the gate. The gate is structured as **decide-then-persist**: `_decide_dod` computes the (status, `DOD_EVALUATED` payload) purely — no DB writes — and is *total*, catching any unforeseen error and failing closed to an `evaluator_error` decision; if normal event building fails, it returns a fixed-shape minimal error event without re-entering the failed builder. `_write_final` persists the event **and** the run status in one transaction and updates only while the run remains non-terminal; if `CANCELLED` was committed while the async judge was in flight, the staged event is rolled back and cancellation remains final. If a `DOD_EVALUATED` insertion fails while the run row is writable, it rolls back and resolves the run status-only as `FAILED` without re-invoking the failed writer. If the fallback status write is itself unavailable, it raises `TerminalPersistenceError` so the caller or recovery layer sees the unresolved row rather than a false success. The judgement makes no tool calls and its free-text reasons are never persisted; its token usage (an immutable `LLMUsage` whose counts are non-negative **ints** by construction — `bool`, `float`, and `NaN` are rejected, not just negatives, since `NaN > cap` is False and would make the cap un-triggerable; `add` validates each *delta* so a `-100` can't net against a positive; and the engine rebuilds it through `LLMUsage` at the gate so a corrupted/duck-typed usage is rejected there too) is **accumulated across every attempt** (retry/fallback usage from an attempt that later failed still counts — usage is folded in *before* the secret gate and validation, so even a rejected response is billed; `max_output_repairs` is one budget for the whole logical judgement, not reset per entry). That usage is **metered against `guards.max_tokens_total`** at the gate — node tokens so far plus the judgement's — before the run may COMPLETE, so a judgement can't slip a run past its hard cost cap; a judgement that *fails outright* still reports the tokens it spent (via a `ProseJudgementError` carrying partial usage) so its cost is recorded, not lost — and the cap check applies to that spend too: a failed judgement whose tokens breach `max_tokens_total` is recorded as `cost_cap_exceeded` (the governance-salient outcome — a repair loop burning past the budget is a cost event, not just an evaluator hiccup), taking precedence over `evaluator_error`. The `DOD_EVALUATED` event carries a durable `outcome` (`met` / `criteria_unmet` / `evaluator_error` / `cost_cap_exceeded`), the per-criterion results, and the judgement usage — enough to tell an unmet criterion from a provider failure without infra logs; the `evaluator_error` cause persists only the exception **class**, never its (possibly credential-bearing) text. The event and the run's terminal status are committed **atomically** (`_write_final` writes both and commits once), so a crash can't strand a `RUNNING` run under a terminal DoD event, and a `DOD_EVALUATED` is recorded on every path the write succeeds. Finalization is also **idempotent**: once a run is in a terminal state (`COMPLETED`/`FAILED`/`CANCELLED`), a re-entry (a second drain, a stray resume) no-ops rather than re-running the judge — which would double-bill, append a second `DOD_EVALUATED`, and could flip a `COMPLETED` run to `FAILED` on a non-deterministic verdict. A DoD whose `evaluated_by` names an unknown agent is rejected at **compile** time, not at the terminal. *(DoD judgement is deliberately NOT recorded as a `node_execution` row — that table is a graph-node dispatch attempt; folding per-agent/provider/model DoD cost into §9's node-level dashboards awaits a first-class LLM-call accounting ledger, a Phase-1 item.)* When no verdict is wired (the mock backend, or a run started without one), prose criteria are recorded as *unevaluated* (advisory — surfaced in the DOD event, not gating), rather than silently passed or hard-failing every run.
...
1. **Structured output is enforced by the strongest mechanism each provider target actually supports — not one strategy for everyone.** The LLM Gateway's provider adapter declares capability flags (`guided_decoding`, `native_structured_output`) for an immutable `(provider, model, endpoint)` target. Strategy memoization includes that full target plus adapter identity, because two OpenAI-compatible endpoints serving the same model name may support different mechanisms:
...
- **Transient failures** (LLM 429/5xx, tool timeout, sandbox cold-start): retry the `node_execution` attempt with exponential backoff, bounded by `guards.max_retries_per_node` (default 3). Each retry is a new row in `node_execution` (same `run_id`/`node_id`, `attempt + 1`). *(Phase 0b: backoff is real at both retry sites — the engine's per-node retry (base 1s doubling per **consecutive** failure — keyed on the failure streak, not the node's lifetime attempt count, so a §3.7 loop revisit doesn't inflate a first failure's delay; cap 30s; and no backoff is spent on a failure that already exhausts `max_retries_per_node` — the run fails immediately) and the gateway's same-fallback-entry retry (base 1s, cap 10s — the inner loop, so smaller than the engine's). Retries are **typed**: adapters classify every provider failure as *retryable* (429/5xx/timeout/connection — a same-entry retry can plausibly succeed) or *permanent* (auth 401/403, invalid request 400/422, unknown model 404 — it cannot). A permanent failure skips the same-entry retry and its backoff entirely and moves straight to the next fallback entry (a different provider may work); a chain whose every entry **ended** *permanently* — judged on each entry's terminal outcome, so an entry whose first failure was a 500 but whose retry died on a 401 counts as permanent — surfaces as a hard error the engine fails the run on immediately, never as a transient the engine would burn `max_retries_per_node` re-running. Client *construction* failures (a missing credential raising at SDK-client init) normalize inside the same boundary as completion failures, classified permanent — a raw SDK exception must never skip the fallback chain. Equal jitter (half fixed, half uniform-random) de-synchronizes concurrent retriers while keeping a deterministic `exp/2` floor. Moving to the NEXT fallback entry does not back off — it's a different provider, and waiting would only delay the recovery the chain exists for. The waiter is injected (`asyncio.sleep`-shaped) so tests observe delays instead of spending wall-clock time.)*
- **Non-transient failures** (repair budget exhausted, tool auth failure, sandbox crash, guard exceeded): `node_execution.status = FAILED` → `run.status = FAILED`. Full `shared_state`, message trail and error are preserved (nothing is rolled back) so an engineer can inspect and manually resume from a specific node rather than restart the whole `Run`. *(Phase 0b: enforced by type — repair exhaustion and the bounded-turn ceiling raise `AgentOutputError` (not a transient), so the run fails immediately instead of spending `max_retries_per_node` node retries. Tool failures are typed on `ToolkitError`: transport timeout / HTTP 5xx / 429 / 408 are **transient** (the engine retries with the same logical visit, so re-issued side effects dedupe on the same invocation key); HTTP 401/403 is **fatal**; everything else (404/422, invalid arguments, bad path) is **model-addressable** and is fed back as an error tool result.)*
- **Idempotency is a Connector SDK requirement, not optional — and identity belongs to a logical invocation, not merely command content.** Each graph entry into a node receives a `logical_visit_id`; retries and HITL resume keep it, while a later graph-loop entry gets a fresh one. Calls within the visit receive a stable ordinal. The key is **`hash(run_id, node_id, logical_visit_id, tool_call_ordinal, tool_name, canonical_json(arguments))`**. This preserves the key across retries, distinguishes two intentional identical calls, and prevents a later loop visit from replaying an earlier result. The key and its visit/ordinal are attached to `message.tool_calls` (§2.2). Any connector with external side effects MUST also forward/dedupe on it because a process can still crash after a remote side effect but before recording local success.
...
- **Secrets never live in workflow/agent config or successful output.** `auth_ref` (toolkit), `llm.api_key_ref` (agent) are pointers resolved by the Agent Runtime at dispatch time and injected only at the provider/HTTP seam. Provider responses, tool results, and the final `AgentTurnResult` cross a fail-closed Secret Output Gate before persistence: exact in-scope credentials and known secret patterns raise a sanitized security error; Ravana never silently edits a state delta because that could change routing. Credential-bearing subprocess stderr cannot bypass that gate, so local MCP child stderr is not forwarded directly to the host process. Structured logging recursively redacts known patterns as a final observability backstop.
- **Prompt-injection surface**: content returned by tools (web search results, DB rows, fetched files, MCP server tool output — §1.7) is untrusted input. The Prompt Assembler must wrap/tag tool output distinctly from system/developer instructions, and an agent's tool permissions are scoped per node — e.g. Dev's `git_connector` token should be repo-scoped, not org-admin-scoped, so a prompt-injected instruction can't escalate blast radius even if it manipulates the model. MCP servers add a variant of this worth calling out separately: a remote server's tool list can change after it was approved ("tool poisoning"), so `mcp_server`-type Toolkits must be registered from an admin-curated allow-list of endpoints, never an arbitrary URL a workflow author pastes in. At the Local tier that allow-list is **`mcp.allowed_servers` in `.ravana/config.yaml`** — the *install* config, deliberately not the workflow file, since a workflow is editable by anyone who can author one. Each entry is a **complete** launch definition (`command`, `args`, `cwd`, `env`, `auth_env`, `read_only_tools`); a workflow selects one by name and contributes nothing to how the process starts, because otherwise an allow-listed interpreter plus workflow-supplied `args` is arbitrary local command execution. It **fails closed**: with no allow-list configured, every `mcp_server` Toolkit is refused rather than any being allowed. `env.PATH` entries must be absolute — a relative one would resolve the command against a different directory than the one validated. See [EXAMPLES.md](EXAMPLES.md) for the concrete YAML.
...
- **Connector SDK security contract**: every connector must (a) declare an input/output JSON schema, (b) accept and dedupe on the logical-invocation `idempotency_key` (§3.6 — visit + ordinal + command content, stable across retries but distinct for intentional repeats) for anything with external side effects, (c) never receive raw secret references — only resolved short-lived credentials the runtime injects, and (d) reject any response that echoes those credentials before it becomes model input.
...
### 8.1 Tenant, User & Run Context Isolation

Cloud execution is **default-isolated, explicit-share**. Execution context is scoped primarily by `run_id` inside a hard `org_id` tenant boundary; it is never assembled by searching for "the same user" and combining prior conversations. Authorization may let an Operator inspect another user's Run in the same organization, but that does not merge either Run's prompt, state, memory, workspace, or credentials.

| Boundary | May be shared intentionally | Must remain isolated by default |
|---|---|---|
| Different organizations | Nothing except explicitly public catalog content | Workflows, Runs, messages, state, HITL, artifacts, memory, credentials, usage and billing |
| Users in one organization | Published Workflow/Agent/Skill/Toolkit definitions according to RBAC; an authorized Operator may inspect or act on a Run | Personal drafts/preferences/auth sessions; execution context is never copied between users merely because they share an organization |
| Runs owned by the same user or organization | Explicit input/artifact references and an explicitly configured long-term-memory scope | `shared_state`, message transcript, short-term memory, HITL responses, repository workspace, sandbox filesystem and tool credentials |
| Long-term memory | Opt-in `user` or `org` scope with provenance and access checks | No implicit cross-run retrieval; never cross `org_id` |

Enforcement is defense in depth:

1. Authentication resolves `user_id` and `org_id` server-side; request bodies cannot choose or override tenant scope.
2. Every tenant-owned query is scoped by `org_id`, with Postgres Row-Level Security as a second barrier rather than the only barrier.
3. Cache keys, secret paths, object-storage prefixes, vector-memory collections and usage events include `org_id`; Run-scoped data also includes `run_id`.
4. A worker receives a signed/leased `{org_id, run_id, node_execution_id}` job, revalidates ownership before loading context, and the Prompt Assembler reads only that Run's state/transcript plus explicitly-authorized shared definitions or memory.
5. Every Run receives a distinct repository workspace and sandbox. No API/worker process mounts a user's live checkout, and no sandbox is reused across tenants.
6. Cross-tenant negative tests cover every API/resource type; context-contamination tests prove concurrent Runs cannot observe one another through prompts, caches, artifacts, logs, usage records or provider-client routing.
...
- **Cost governance vs. billing — two different problems that share the same data.** Governance is capping: `guards.max_tokens_total` (already in the YAML) is enforced by the orchestrator tallying token usage per `run` after every node commit, failing the run loudly rather than silently truncating output, with per-`org_id` daily/monthly spend caps a layer above that. Every token count that enters that tally — a node turn's *and* a DoD judgement's — is validated as a non-negative int (via `LLMUsage`) at the engine boundary before it is summed or persisted, so a runtime reporting a negative/`NaN` count can't poison the running total and slip the run past its cap (it fails the turn closed instead). A node turn is also *snapshotted* into a trusted plain result — each field read exactly once at the boundary — so a hostile/duck-typed runtime object can't report one token count at the cap check and a different one at the DB write (a time-of-check/time-of-use poisoning); every downstream read (secret gate, guards, cap, persistence) is of the snapshot, never the live object. Billing is charging: rolling up `node_execution.estimated_cost_usd` (§2.2) per `org_id` per billing period into a metered-billing integration (e.g. Stripe) is what actually turns the Managed Cloud tier (§10) into a business — distinct from governance, needed starting whenever that tier ships (§12 Phase 3), not before.
...
- **Infrastructure/application log — this was the actual gap.** Structured JSON logs from the Orchestrator/Agent Runtime processes themselves (lease claimed/released, LLM HTTP call timing and retries, sandbox lifecycle, unhandled exceptions) — "how the system is behaving," distinct from "what the agents did." Two things make this useful instead of just noise:
  ...
  - **The infra logger is best-effort and must never break the run.** It is called from inside the engine's fail-closed terminal paths, so a write failure (a closed/broken stderr, a full disk) that propagated would escape *past* the run-status commit and strand the run at `RUNNING` — the exact opposite of what a log line is for. `log_event` swallows its own serialization/write errors: losing a log line is strictly better than losing the run. Crucially, its *fallback* path (used when an `extra` field can't serialize) still runs the record through the §8 redaction backstop — a serialization failure must never route a secret in the message around redaction — and if even the minimal record can't serialize, it emits nothing rather than raw text. (The durable *domain* log — the `state_transition_log` row — is a DB write inside the run's transaction and is not best-effort; it commits atomically with the run status.)
...
my-project/                  <- YOUR actual working directory — agents never write here
  ...
  .ravana/
    ...
    runs/<run_id>/
      workspace/                an isolated `git clone --no-hardlinks` of my-project, on branch `ravana/run-<run_id>` —
...
1. **`git clone --no-hardlinks <repo toplevel> .ravana/runs/<run_id>/workspace` on a fresh branch off the requested base** (default; `ravana run start --base-ref <ref>`, with `HEAD` as the default). A local clone is a fully independent repository — separate `.git`, separate refs, separate index/HEAD — so there is categorically no way for anything the agent does (including a bad `git reset`/`rm -rf`, not just a bad file edit) to reach your actual checkout. **`--no-hardlinks` is load-bearing, not an optimization to skip**: the sandbox bind-mounts the workspace *including its `.git`* read-write (point 4), so plain `--local` hardlinking — which shares `.git/objects` inodes with the source — would let agent code writing through a workspace object inode corrupt the developer's own repo (its `git fsck` then fails). Copying the objects costs more disk/time than a hardlink but is the price of the isolation guarantee. Provisioning clones into a staging dir and `rename`s it into place atomically, so an interrupted attempt never leaves a half-clone a retry would mistake for a finished workspace. A provenance record stored beside the workspace records the isolation mode, source toplevel, branch, and resolved base commit; workspaces without a valid record are rejected, so legacy `--local` hardlinked clones cannot be reused. An existing workspace is reused only when it is itself the Git toplevel, is on the expected run branch, and has the expected source origin. The source is resolved to its repo **toplevel** (`git rev-parse --show-toplevel`), so a `.ravana/` in a monorepo subdirectory clones the whole repo while retaining that subdirectory as the sandbox working directory.
...
4. `code_interpreter`'s local Docker container (§8's build-vs-buy tier for this mode) mounts **only `runs/<run_id>/workspace`**, never the parent project directory. The engine invokes the runtime's run-preparation capability before persisting/dispatching a Run, and the code-interpreter handler owns Git provisioning rather than relying on one CLI call site. For a nested monorepo project, the mount remains the repo clone but the container working directory is constrained to `/workspace/<project-relative-path>`. The isolation is enforced at the bind-mount level, not just by convention, so it holds even if something inside the container misbehaves.

**Handoff back to the real project is a deliberate, reviewable step, not an automatic one.** When the PM's final review marks a run `COMPLETE` (§4's `pm_final_review` node), the workspace branch is surfaced for review — never an auto-merge into the developer's checked-out branch. *Implemented today:* a `git format-patch` series (plus an `uncommitted.diff` when the agent left work uncommitted) written to `runs/<run_id>/artifacts/handoff/`, based on the commit recorded in the workspace provenance and applied by the developer with `git am` at their discretion. The never-touch-the-source guarantee is structural, not procedural: every git invocation on this path is scoped `-C <workspace>`, so handoff cannot merge, rebase, or check anything out in the real repo. Delivery never adjudicates the run — the outcome is already committed, so a failure to write the patch is logged, never converted into a run failure. *Not yet implemented:* pushing the branch and opening a pull request when a remote is configured. This is the same "human approves the boundary crossing" pattern already used for HITL (§3.1) and for publishing a workflow version (§2.2), applied to the one place an agent's output actually reaches source control.
...
The cloud destination is a multi-user, multi-workflow control plane with two execution choices: ephemeral cloud sandboxes for repositories reachable through a Git provider installation, and an outbound-only Local Runner for private/on-prem workspaces. Both feed the same durable Run model. Usage must be captured as immutable, deduplicated events from the first hosted phase, even though customer charging waits until Managed Cloud; otherwise retries, failed calls, historical prices, and BYOK cannot be reconciled reliably later.
...
| **1 — Self-hosted (single instance) & UI** | Add the **self-hosted single-instance tier** (§10.2): `docker compose up` bundles Postgres + Redis and exercises the lease/CAS path. Add authenticated user/org identities (single-org operation is acceptable here), Run-scoped context assembly, the complete Ravana Console, Git-provider installation credentials for isolated cloud repository clones, optional managed sandboxes, and append-only usage-event collection with immutable price snapshots. Usage is visible per Run but is not invoiced yet. | Strict multi-tenant enforcement/RBAC, hybrid Local Runner, Vault, eval harness, Kubernetes, customer charging. |
| **2 — Production hardening (scaled self-hosted)** | Move to the **scaled tier** (§10.3, Kubernetes/Helm). Enforce §8.1 with API/query tenant scope, Postgres RLS, namespaced caches/secrets/storage/memory, one sandbox per Run, and cross-tenant/context-contamination tests; apply RBAC at every control-plane boundary. Add the outbound-only Hybrid Local Runner for private/on-prem repositories, per-org quotas/backpressure, credential-level rate limits, usage dashboards, budget enforcement, pricing-version history, and provider-cost reconciliation. Golden-run eval gating, pgvector memory, and justified workflow composition land here too. | Multi-region, Kafka, invoices/payments. |
| **3 — Scale-out / Managed Cloud** | Add Kafka/multi-region throughput, data governance/right-to-delete, and the SOC2 program. Turn the Phase 1-2 usage ledger into commercial billing: plan allowances, prepaid credits or metered charges, BYOK-vs-platform pricing, customer line items, invoices/payments/refunds, and billing-provider webhook reconciliation. Managed Cloud otherwise runs the same Phase 2 control/execution architecture under vendor operation. | — |
