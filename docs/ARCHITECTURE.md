...
Version: 0.15 (Draft) · Author: Principal Architecture Draft · Date: 2026-07-06
...
CREATE TABLE workflow_node (
    ...
    on_enter        TEXT,                         -- state-mutation expression run on entry
...
CREATE TABLE run (
    ...
    current_nodes   TEXT[],                       -- supports concurrent active nodes
...
7. **Terminate** — When a node's outgoing edge targets `__terminal__` (or `dod_criteria` is satisfied), `run.status = COMPLETED`; for a sub-workflow run, `COMPLETED` also triggers the parent node's `output_map` (§1.8) to commit into the parent's `shared_state`, unblocking the parent's own step 5. If `guards.max_total_steps` is exceeded or an unhandled error occurs, `run.status = FAILED`.
...
- **Transient failures** (LLM 429/5xx, tool timeout, sandbox cold-start): retry the `node_execution` attempt with exponential backoff, bounded by `guards.max_retries_per_node` (default 3). Each retry is a new row in `node_execution` (same `run_id`/`node_id`, `attempt + 1`).
