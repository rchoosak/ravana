...
7. **Terminate** — When a node's outgoing edge targets `__terminal__`, **or a node has no outgoing edges at all** (an *implicit terminal* — equivalent to an explicit `to: [__terminal__]` edge, logged as a TERMINATE event; previously undefined, which produced a real stuck-`RUNNING` bug in Phase 0a), `run.status = COMPLETED`; for a sub-workflow run, `COMPLETED` also triggers the parent node's `output_map` (§1.8) to commit into the parent's `shared_state`, unblocking the parent's own step 5. If `guards.max_total_steps` is exceeded or an unhandled error occurs, `run.status = FAILED`. **`dod_criteria` as a termination trigger is design intent not yet implemented** — Phase 0a terminates on graph structure only; evaluating DoD needs an evaluator mechanism (rule-based for expression criteria, an agent turn for prose criteria like §4's "all acceptance criteria met"), which is an open design question tracked as a Phase 0b item in TASKS.md.
